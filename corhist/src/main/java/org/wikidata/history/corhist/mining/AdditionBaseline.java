...
class AdditionBaseline {
  ...
  private Optional<Statement> buildStatement(ConstraintViolationCorrection correction) {
    ...
    switch (constraint.getType().getLocalName()) {
      ...
      case "Q21503250":
        //Type
        return getRelationParameter(constraint).flatMap(propertyToHave -> constraint.getParameter(QueriesForConstraintCorrectionsBuilder.CLASS_PARAMETER).map(type ->
                valueFactory.createStatement(
                        correction.getTargetTriple().getSubject(),
  ...
  private Optional<IRI> getRelationParameter(Constraint constraint) {
    return constraint.getParameter(QueriesForConstraintCorrectionsBuilder.RELATION_PARAMETER).flatMap(relation -> {
      if (relation.equals(QueriesForConstraintCorrectionsBuilder.INSTANCEOF_ENTITY)) {
        return Optional.of(QueriesForConstraintCorrectionsBuilder.INSTANCEOF_PROPERTY);
      } else if (relation.equals(QueriesForConstraintCorrectionsBuilder.SUBCLASSOF_ENTITY)) {
        return Optional.of(QueriesForConstraintCorrectionsBuilder.SUBCLASSOF_PROPERTY);
      } else {
        return Optional.empty();
      }
    });
