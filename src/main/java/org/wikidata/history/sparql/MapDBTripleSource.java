...
final class MapDBTripleSource implements TripleSource, AutoCloseable {
  ...
  private final MapDBStore.IndexReader<Long, Integer> revisionTopicIndex;
  private final MapDBStore.IndexReader<Integer, long[]> topicRevisionsIndex;
    ...
    if (Vocabulary.HISTORY_GLOBAL_STATE.equals(pred)) {
      ...
      if (subj == null) {
        if (obj == null) {
          ...
          long[] revisions = topicRevisionsIndex.getOrDefault(parseEntityIRI(obj), EMPTY_ARRAY);
          if (revisions == null) {
            LOGGER.error("revisions shall not be null");
        ...
        if (obj == null) {
          return Optional.ofNullable(revisionTopicIndex.get(parseRevisionIRI(subj)))
                  .map(valueFactory::createEntityIRI)
                  .map(newObj -> toIteration(subj, pred, newObj))
                  .orElse(EMPTY_ITERATION);
        } else {
          return revisionTopicIndex.getOrDefault(parseRevisionIRI(subj), 0) == parseEntityIRI(obj)
                  ? toIteration(subj, pred, obj)
                  : EMPTY_ITERATION;
        }
      }
  ...
  private int parseEntityIRI(Value entityIRI) {
    if (!(entityIRI instanceof NumericValueFactory.ItemIRI || entityIRI instanceof NumericValueFactory.PropertyIRI) && entityIRI instanceof IRI) {
      entityIRI = valueFactory.createIRI((IRI) entityIRI);
    }
    if (entityIRI instanceof NumericValueFactory.ItemIRI) {
      return ((NumericValueFactory.ItemIRI) entityIRI).getNumericId();
    } else if (entityIRI instanceof NumericValueFactory.PropertyIRI) {
      return -((NumericValueFactory.PropertyIRI) entityIRI).getNumericId();
    } else {
      throw new QueryEvaluationException("Not supported entity IRI: " + entityIRI);
    }
  }
  ...
  private Statement formatTriple(NumericTriple triple, Resource context) {
    try {
      return valueFactory.createStatement(
              valueFactory.createEntityIRI(triple.getSubject()),
              triple.getPredicate() == -1
                      ? OWL.SAMEAS
                      : valueFactory.createDirectPropertyIRI(triple.getPredicate()),
