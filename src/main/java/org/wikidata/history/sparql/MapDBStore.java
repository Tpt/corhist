...
class MapDBStore implements AutoCloseable {
  ...
  Index<Long, Integer> revisionTopicIndex() {
    return new Index<>(directory.resolve("revision_topic.db"), Serializer.LONG_PACKED, Serializer.INTEGER);
  }

  Index<Integer, long[]> topicRevisionIndex() {
    return new Index<>(directory.resolve("topic_revision.db"), Serializer.INTEGER, Serializer.LONG_ARRAY);
  }
